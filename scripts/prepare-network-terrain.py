"""Fetch and bilinearly sample Mapzen Terrarium DEM tiles for the Western European study.
...
assert len(heights) == SIZE * SIZE and all(-12000 < h < 6000 for h in heights)
...
(ROOT / 'data/processed/network-terrain.json').write_text(json.dumps(data, separators=(',', ':')) + '\n')
provenance = {'source': 'Mapzen Terrain Tiles on AWS', 'accessed': '2026-09-12',
              'registry': 'https://registry.opendata.aws/terrain-tiles/',
              'attribution': 'Europe terrain produced using Copernicus data and information funded by the European Union — EU-DEM layers; SRTM and GMTED2010 courtesy of the U.S. Geological Survey; Austria terrain © offene Daten Österreichs — Digitales Geländemodell (DGM) Österreich. Norway terrain © Kartverket; UK terrain © Environment Agency copyright and/or database right 2015.',
              'attributionReference': 'https://github.com/tilezen/joerd/blob/master/docs/attribution.md',
              'processing': 'Decode R*256+G+B/256-32768 metres before bilinear interpolation; sample at 4.6875 km spacing; round to metres. No invented relief.',
              'rangeMetres': [min(heights), max(heights)],
              'centreElevationMetres': heights[len(heights)//2], 'tiles': sources}
(ROOT / 'data/provenance/network-terrain.json').write_text(json.dumps(provenance, indent=2) + '\n')
print(f'{SIZE} × {SIZE} grid; {min(heights)}–{max(heights)} m; centre terrain {heights[len(heights)//2]} m; {len(tiles)} tiles')
