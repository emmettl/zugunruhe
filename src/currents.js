...
document.querySelector('#continent-app').innerHTML=`
  <header><a class="identity" href="/">ZUGUNRUHE<span>MOTION STUDIES / 04</span></a><span class="header-place">Western Europe · 4–5 September 2018</span><a class="previous" href="/studies/02-archipelago/">Archipelago ↗</a></header>
  <main><div id="world"></div>
    <div class="title"><span class="eyebrow">CURRENTS</span><h1 id="view-title">The night has a direction.</h1><p id="view-subtitle">Follow the light across the landscape.</p><button id="back-to-network" hidden>← Back to previous view</button></div>
    <div class="inspector"><label for="station">Visit a station</label><select id="station"><option value="-1">Whole field</option>${network.stations.map((s,i)=>`<option value="${i}">${stationName(s)} · ${s.name}</option>`).join('')}</select><div class="palette-control"><label for="palette">Palette</label><select id="palette">${Object.entries(palettes).map(([id,p])=>`<option value="${id}">${p.label}</option>`).join('')}</select></div><label class="inspection"><input id="threads" type="checkbox" checked> Travelling threads</label><div id="reading"></div><label class="inspection"><input id="inspection" type="checkbox"> Show observation support</label><div class="spectrum" id="spectrum"></div><div class="spectrum-key"><span id="legend-low">1 km</span><span id="legend-title">Altitude</span><span id="legend-high">4 km</span></div></div>
  ...
  <section id="notes" hidden><button id="close-notes" aria-label="Close study notes">×</button><h2>Following the night.</h2>
    <p>These luminous threads follow paths calculated through radar-derived bird velocities. They are population-flow tracers, not tracked individual birds, flocks or confirmed migration routes. The <a href="continent.html">Sea study</a> remains available as the earlier continuous field.</p>
    <p>Each thread persists as the field changes. East/north velocities are estimated on the same 0.25° grid, interpolated in space and time, and integrated in one-minute steps. Travel distance follows the data clock: five real minutes pass in one screen second. The tail holds up to 75 minutes of earlier positions, fading toward its oldest end.</p>
    <p>Seeds favour areas with higher estimated density, across the fifteen altitude bands. Their number, lifetime, width and glow are display choices, not bird counts. A path holds its altitude; no vertical movement is inferred. Paths stop when velocity support runs out or terrain reaches their altitude. They do not wrap around the map or bridge missing observations.</p>
    <p>The paths are prepared deterministically for this night. Pausing freezes their motion; scrubbing restores the same paths at any chosen time. Space or P plays and pauses, arrows move five minutes, Shift + arrows thirty minutes, and Home/End visit the endpoints.</p>
...
function updateUI(){
  ...
  if(selected<0){$('reading').innerHTML='<strong>37 <small>stations</small></strong><p>Radar-derived currents<br>Trails hold the previous 75 minutes</p>';return;}
...
$('palette').value=Object.hasOwn(palettes,requestedPalette)?requestedPalette:'aquatic';
applyPalette(true);
...
scene?.setGain(Number($('luminosity').value));
$('threads').addEventListener('change',()=>scene?.setThreads($('threads').checked));
