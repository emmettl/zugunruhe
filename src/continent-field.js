...
export function createContinentalField(scene,landscape){
  ...
  const uniforms={fieldA:{value:makeTexture()},fieldB:{value:makeTexture()},fraction:{value:0},time:{value:0},
  ...
  const material=new THREE.ShaderMaterial({uniforms,transparent:true,depthWrite:false,side:THREE.DoubleSide,blending:THREE.AdditiveBlending,
    ...
    fragmentShader:`uniform sampler2D fieldA,fieldB;uniform float fraction,time,gain,viewExposure,inspection;varying vec2 vUv,vMap;varying float vBand,vGround;
      ...
      vec3 colour(float b){float t=b/14.;if(t<.333)return mix(vec3(1.,.53,.20),vec3(1.,.20,.49),t*3.);if(t<.666)return mix(vec3(1.,.20,.49),vec3(.35,.22,1.),(t-.333)*3.);return mix(vec3(.35,.22,1.),vec3(.19,.85,.85),(t-.666)*3.);}
  ...
  return {uniforms,setIndex(index){const a=Math.floor(index),b=Math.min(144,a+1);if(left!==a){uniforms.fieldA.value.image.data=frame(a);uniforms.fieldA.value.needsUpdate=true;left=a;}if(right!==b){uniforms.fieldB.value.image.data=frame(b);uniforms.fieldB.value.needsUpdate=true;right=b;}uniforms.fraction.value=index-a;},
    draw(dt,playing){if(playing)uniforms.time.value+=dt;}};
}
