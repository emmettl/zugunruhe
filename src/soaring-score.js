...
export function soaringMusic({ height = 150, climb = 0, mode = 'seeking' } = {}) {
  const rising = clamp(climb / 4, 0, 1), altitude = clamp((height - 150) / 400, 0, 1);
  return { interval: 8 - rising * 3, release: mode === 'gliding' ? 9 : 6,
    brightness: .04 + altitude * .14, spread: mode === 'gliding' ? .65 : .25,
...
export function createSoaringSound({ createContext, schedule = setInterval, unschedule = clearInterval }) {
  ...
  function tick() {
    ...
    drones.forEach(({ gain, level }, i) => gain.gain.setTargetAtTime(level * (.8 + .2 * Math.sin(breath + i * .3)), now, .7));
    ...
    const leaving = departure !== null && event !== departure && state.bird === state.previousBird && state.mode === 'gliding';
    departure = event;
    if (leaving || progress >= 1) {
      progress = 0;
      const melody = state.mode === 'climbing' ? [62, 64, 66, 69, 74] : [69, 64, 57, 66];
