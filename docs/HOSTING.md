...
hostname-guarded GitHub Pages analytics beacon as the peer editions. The five
current entries receive Cloud / Islands / Sea / Currents / Night navigation.

`pages.yml` calls `checks.yml`: locked installation, unit tests, build and
Chromium/iPhone-WebKit smoke tests of all seven study entries under `/zugunruhe/`.
