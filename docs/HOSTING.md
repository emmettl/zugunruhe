...
`continent.html` for the sea, `currents.html` for directional paths, `night.html` for the continuous journey, and `air.html` for regional birds and wind; `air-station.html` retains the Memmingen comparison. Preserved builds are under `studies/01-layers/`
...
hostname-guarded GitHub Pages analytics beacon as the peer editions. Current entries receive Cloud / Islands / Sea / Currents / Night / Air navigation.

`pages.yml` calls `checks.yml`: locked installation, unit tests, build and
Chromium/iPhone-WebKit smoke tests of all study entries under `/zugunruhe/`.
